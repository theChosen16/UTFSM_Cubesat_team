...
- **Verified institutional addresses (the workspace boundary was a self-assertion)**: `isInstitutional()` checked the *shape* of the token's e-mail claim and nothing else, and that claim proves nothing on its own. `identitytoolkit.googleapis.com/v1/accounts:signUp` is a public endpoint keyed by the Firebase Web API key, which necessarily ships inside the client bundle of a public GitHub Pages site; it neither verifies the address nor requires the caller to receive anything at it. So **any anonymous person on the internet** could mint a token for an `@usm.cl` / `@sansano.usm.cl` address they do not own — no institutional mailbox, no relationship with the university, no interaction with the team — and immediately read the entire "private" workspace: every project, task, file record, calendar event, post, chat message and audit entry, plus `system_config/keys`, which hands them the Drive bridge endpoint and shared secret. The client-side domain check in `Register.tsx` never entered the picture, because nothing forces an attacker through the UI. Membership now requires `email_verified` on the token, which is the part that actually proves control of an institutional mailbox:
  - `AuthContext.signUp` sends the verification e-mail at registration, parks the name typed during sign-up on the Firebase Auth record, and — critically — **no longer writes the `/users/{uid}` profile**. The profile is provisioned on the first sign-in that carries a verified token, and `/users` `create` is gated on `hasVerifiedInstitutionalEmail()`. Provisioning is what makes an account a member, so it cannot happen before ownership is proven.
  - `ProtectedRoute` renders a verification step (resend / "ya verifiqué", which forces an ID-token refresh, since `email_verified` is a claim baked in at issue time and would otherwise stay stale for up to an hour) instead of a workspace full of permission errors.
  - **Migration, not lockout:** `isInstitutional()` also admits a caller whose `/users/{uid}` document already exists. That clause is not a hole — creating that document requires a verified address, so an unverified newcomer cannot grandfather themselves — but it keeps every member who registered before this change working while they verify at their own pace. They see a persistent banner. Once the roster is fully verified the `exists()` clause can be deleted outright.
  - Verified against the Firestore emulator: an unverified `@usm.cl` account is denied `system_config`, `tasks` and `posts` and cannot create its own profile; the same account is admitted after redeeming the verification link; a pre-existing profile still grants access. Regression tests in `src/test/e2e/security-rules.e2e.test.ts`, with the whole e2e fixture set moved onto a `createVerifiedUser()` helper that redeems a real out-of-band code.
- **Notifications: every create-side control was bypassable through `update`**: the `type` allowlist, the `system`-alert gate, `isTruthfulSenderName()`, the size caps and the `senderId == uid` pin were all enforced on `create` only, while `update` authorized *any* change to a document whose **pre-image** named the caller as recipient. Two writes defeated the lot: create a perfectly legitimate `message` notification addressed to **yourself** (you are both sender and recipient, so every create check passes), then update it — rewriting `recipientId` to the victim, `type` to `system`, `senderName` to "Maestro USM CubeSat" and the body to whatever you like. The read rule keys on the *current* `recipientId`, so the forged document lands in the victim's inbox and renders as an official platform alert signed by someone with authority: precisely the in-app phishing the `system` gate and the sender-name check exist to prevent, reached through the back door. `update` is now confined to what the client actually performs (`NotificationService.markAsRead` writes `{ read: true }`): the recipient must match on **both** sides of the write and `read` is the only field a write may touch, which makes every identity-bearing field immutable after creation. Regression test in `src/test/e2e/security-rules.e2e.test.ts`.
- **The `/mail` relay was still open through `cc`/`bcc` and friends**: constraining `to` to the institutional pattern only closed one of several recipient fields the Firebase *Trigger Email* extension honours. It also resolves `cc`, `bcc`, `toUids`, `ccUids` and `bccUids`, and acts on `from`, `replyTo`, `headers`, `template` and `message.attachments` — whose `path`/`href` form makes the extension fetch an arbitrary URL and attach the response. None of those were mentioned by the rule, so a `manager` (a self-service **team**, granted by any admin — not a vetted role) could enqueue an institutional `to` alongside `bcc: ["victim@example.com"]` and have attacker-authored HTML delivered anywhere on the internet from the team's own SMTP identity, passing its SPF/DKIM, with a spoofable `from`. The rule now pins the **document shape** to exactly what `EmailNotificationService.sendWeeklyDigest` writes (`to`, `message.{subject,html,text}`, `createdAt`) rather than blocklisting fields one at a time, so a future extension feature cannot silently reopen the hole. Regression test in `src/test/e2e/security-rules.e2e.test.ts`.
- **The weekly digest interpolated an untrusted event field unescaped**: `EmailNotificationService.generateHTMLTemplate` escapes `titulo`, `descripcion` and the greeting, but the date went through `formatDate`, whose error fallback returned the raw string straight into the mailed HTML. `fechaInicio` is not trusted data: `AdminActionsService.auditarActaDrive` writes it verbatim from text the model derived from an uploaded meeting minute, so a poisoned document could steer a value into a message that is then mailed to every active member. Both the formatted path and the fallback are escaped now, and the `events` rule bounds `fechaInicio`/`fechaFin` to short strings so an oversized value never reaches the template. Covered by `src/sdk/EmailNotificationService.test.ts`.
- **Avatars and inlined media could not be turned into tracking beacons**: `photoURL`, `posts.imageUrls`, `portfolioImages` and `project_messages.fileUrls` are self-writable fields that the shipped clients only ever fill with `data:` URLs (`FileReader.readAsDataURL`) — but the field accepts any string, and the CSP allowed `img-src ... https:` with a wildcard. Any member could therefore point their avatar at an attacker-controlled URL and turn every page that renders the roster (Miembros, Feed, Dashboard, chat) into a beacon logging the IP address, user-agent and viewing time of each teammate who opened it. `img-src` is now limited to `'self'`, `data:` and the Google Drive hosts where the team's real files live, which neutralises the vector regardless of what ends up stored in Firestore.
- **Manager-writable documents are bounded too**: `projects` (`nombre`, `descripcion`) and the event date fields had no ceiling at all, the last manager-writable documents without one. Since `manager` is a self-service team rather than a vetted role, that surface gets the same storage/egress bounds as the member surface.
- **The AI proxy no longer relays upstream error bodies**: `handleChat` returned Gemini's raw error payload to the caller as `details`. It echoes back request metadata and provider-side detail that only helps someone probing the proxy, and the client has no use for it beyond the status code. The full text now goes to the Apps Script execution log, which only the owner can read.
